...
The X11 bitmap adapter converts accepted Mono1 frames to the server-advertised XYBitmap representation. It handles 8/16/32-bit scanline units, independent image-byte and bitmap-bit order, scanline padding, and whole-row chunking within the server's maximum request size. Unsupported X11 formats or conversion failures are logged without ending the event loop. Prepared frames are still reported as `render=pending`; this step does not upload them to the drawable yet. The line-oriented semantic action path and local Exit remain active until framebuffer rendering is implemented and verified.
