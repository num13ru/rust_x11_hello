//! Embedded five-column bitmap font used by the deterministic mono renderer.

pub(crate) const WIDTH: u16 = 5;
pub(crate) const HEIGHT: u16 = 7;
pub(crate) const VERTICAL_SCALE: u16 = 2;
pub(crate) const ADVANCE: u16 = WIDTH + 1;

pub(crate) fn rows(character: char) -> [u8; HEIGHT as usize] {
    match character {
        ' ' => [0, 0, 0, 0, 0, 0, 0],
...
mod tests {
    ...
    #[test]
    fn mixed_case_digits_and_fallback_have_distinct_glyphs() {
        assert_ne!(rows('A'), rows('a'));
        assert_ne!(rows('1'), rows('9'));
        assert_eq!(rows('\n'), rows('?'));
        assert_eq!(rows('é'), rows('?'));
    }
}
